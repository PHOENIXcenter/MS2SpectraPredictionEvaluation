...
#肽段信息及比对的PCC结果文件
OutputPCCfile = '2019Wang_MSBLysN_HCD/MassAnalyzer-result/MassAnalyzer_all_inten_pears.txt'
#工具预测结果中各位置b,y离子的强度
...
with open(ToolpredictFile, 'r') as f:
	while True:
		...
		if 'ModString=' in line:
			...
			seq_mass = seq_mass + config.mass_H2O + config.mass_H
			pro_mass = 0
			b_inten, y_inten = [], []
			for index, AA in enumerate(seq[:-1]):
				...
				for mz in mz2inten.keys():
					if abs((pro_mass + config.mass_H - mz))/mz*(10**6) <= 20:
						b_inten.append(mz2inten[mz]/max_predinten)
						isb = True
						break
				for mz in mz2inten.keys():
					if abs((seq_mass - pro_mass - mz))/mz*(10**6) <= 20:
						y_inten.append(mz2inten[mz]/max_predinten)
						isy = True
						break
				if not isb:
					b_inten.append(0.0)
				if not isy:
					y_inten.append(0.0)
			title2inten[titleList[num]] = [pepinfos, b_inten, y_inten, b_inten + y_inten]
			num += 1
...
with open(TargetMGF, 'r') as f:
	while True:
		...
		if 'TITLE=' in line:
			...
			mgf_all_ion = mgf_b_inten + mgf_y_inten
			#分别对b离子，y离子计算PCC，再对b,y离子一起计算PCC
			b_pcc = config.pearSim(title2inten[mgf_title][1], mgf_b_inten)
			y_pcc = config.pearSim(title2inten[mgf_title][2], mgf_y_inten)
			by_pcc = config.pearSim(title2inten[mgf_title][3], mgf_all_ion)
			#将所需信息写入列表
			SeqInfos.append(mgf_pepinfo.split('/')[0])
			ChargeInfos.append(mgf_charge)
			pred_b_intens.append(title2inten[mgf_title][1])
			pred_y_intens.append(title2inten[mgf_title][2][::-1])
