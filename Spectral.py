...
#肽段信息及比对的PCC结果文件
OutputPCCfile = 'ninanjie/pDeep-result/pDeep2_all_inten_spectral.txt'
#工具预测结果中各位置b,y离子的强度
	...
	while True:
		...
		if 'TITLE=' in line:
			...
			if mgf_title in title2inten.keys():
				...
				b_pcc1 = config.pearSim(title2inten[mgf_title][1], mgf_b_inten)
				y_pcc1 = config.pearSim(title2inten[mgf_title][2][::-1], mgf_y_inten)
				by_pcc1 = config.pearSim(pred_by_inten, mgf_by_inten)
				b_pcc = 1 - 2*(math.acos(b_pcc1)/math.pi)
				y_pcc = 1 - 2*(math.acos(y_pcc1)/math.pi)
				by_pcc = 1 - 2*(math.acos(by_pcc1)/math.pi)
				#将所需信息写入列表
...
with open(OutputPCCfile, 'w') as f:
	f.write('peptide\tlength\tcharge\tb_ion_spectral\tpredict_b_ion\tmgf_b_ion\ty_ion_spectral\tpredict_y_ion\tmgf_y_ion\tby_ion_spectral\n')
